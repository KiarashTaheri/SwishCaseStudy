...
Every number here is reproducible. The measurements about soiling, the quality gate and the region
partition come from `data/` via `backend/scripts/verify.py`. The scale figures in #4 cannot — `data/`
is twelve plants and four months — so they come from the generator run at 200 plants × 5 years, and
#4 gives the two commands.
...
**Decision.** SQLite for source and derived tables, with hand-written SQL.

*Measured* at the stated end state, generated by the brief's own generator rather than estimated:

```bash
python3 seed_data.py --plants 200 --days 1825 --start 2021-09-16 --out data_5y
PYTHONPATH=. .venv/bin/python -m swishos.build --data ../data_5y --db end_state.db
```

**335,585 daily readings** — 200 plants over five years, short of 365,000 because plants commission
on staggered dates. The whole database is **93.4 MB**, ingest and ranking run end to end in **4.3 s**,
and the fleet query returns its 200 rows in **0.22 ms median, 0.23 ms p95** over 20 runs. None of
that is close to a constraint.

Two things the run shows that an estimate would have missed. First, **retention policy costs more
than the readings do**: a snapshot row is 369 bytes, so keeping one snapshot per day for five years
adds ~135 MB and more than doubles the database, while five years of source readings is 21.8 MB.
Derived data outgrows source data here, and #3's daily snapshot is what needs an expiry rule — not
`daily_reading`. Second, the PR band the quality gate depends on **stays empty at 335,585 plant-days**
(0.3966 below, 0.8443 above, width 0.4477), which is A2b's falsifier fired at 250× the default
dataset and surviving.

The one caveat: 0.22 ms is a warm single-process read. It says the query plan is sound, not that 120
concurrent readers are free.
...
decays and `s₀·T` overstates the gain. The exposure is per plant, not fleet-wide: 9.4% in Andalusia,
where drizzle is commonest and `T = 13` shortest, and zero in Arizona and Atacama, which record no
sub-8 mm rain in 120 days.

**Cost.** Recommendations are optimistic in the regions that drizzle, correct in the ones that do not.

**Falsifier — fired.** A recommendation clearing break-even by less than its own correction; below
that, the thing I chose not to model decides the verdict. `plant_1001` clears by $182 on $12,532 of
gross recovery — 1.5% — against 2.9% for its drizzle rate, and goes to **−$175**. It is also the plant
the interface marks `Clean · thin`, so both signals agree. The refinement: the bound is per plant, not
the fleet-wide ~13% first written here.
...
a day, and has never exceeded 11.74% on a credible reading across 1,137 plant-days. Against that, 29
